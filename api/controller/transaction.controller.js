...
export async function createDeposit(req, res) {
    ...
    try {
        ...
        return res.status(201).json({ success: true, message: 'Deposit request created' });
    } catch (error) {
        console.error('Error creating deposit:', error);
        return res.status(500).json({ error: 'Failed to create deposit', message: error.message });
    }
}
...
export async function getPendingDeposits(req, res) {
    ...
    try {
        const deposits = await Transaction.find({ user_id: userId, type: 'deposit', status: 'pending' });
        return res.status(200).json(deposits);
    } catch (error) {
        return res.status(500).json({ error: 'Failed to fetch pending deposits', message: error.message });
    }
}
...
export async function setDepositStatus(req, res) {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: `Deposit status set to ${status}` });
    } catch (error) {
        res.status(500).json({ error: 'Failed to set deposit status', message: error.message });
    }
}
...
export async function getAllPendingDeposits(req, res) {
    try {
        ...
        return res.status(500).json({ error: 'Failed to fetch all pending deposits', message: error.message });
    }
}